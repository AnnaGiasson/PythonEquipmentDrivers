...
class Intepro_PSI9000(VisaResource):
    ...
    def set_lock(self, state: bool):
        self._resource.write(f"SYST:LOCK {1 if state else 0}")

    def get_lock(self) -> True:
        query = self._resource.query("SYST:LOCK:OWN?")
        if query.strip('\n') == 'REMOTE':
            return True
        return False
    ...
    def get_state(self) -> bool:
        ...
        query = self._resource.query("OUTP?")
        if query.strip('\n') == "ON":
            return True
        return False
    ...
    def get_voltage(self):
        return float(self._resource.query("VOLT?").strip("V\n"))
    ...
    def get_current(self):
        return float(self._resource.query("CURR?").strip("A\n"))

    def set_power(self, power):
        self._resource.write(f"POW {power}")
        return None

    def get_power(self):
        return float(self._resource.query("POW?").strip("W\n"))

    def measure_voltage(self):
        return float(self._resource.query("MEAS:VOLT?").strip("V\n"))

    def measure_current(self):
        return float(self._resource.query("MEAS:CURR?").strip("A\n"))

    def measure_power(self):
        return float(self._resource.query("MEAS:POW?").strip("W\n"))

    def measure_array(self):
        query = self._resource.query("MEAS:ARR?").strip('\n')
        v, i, p = query.split(',')
    ...
    def set_ovp(self, voltage):
        self._resource.write(f"SOUR:VOLT:PROT {voltage}")
        return None

    def get_ovp(self):
        return float(self._resource.query("SOUR:VOLT:PROT?").strip("V\n"))

    def set_ocp(self, current):
        self._resource.write(f"SOUR:CURR:PROT {current}")
        return None

    def get_ocp(self):
        return float(self._resource.query("SOUR:CURR:PROT?").strip("A\n"))

    def set_opp(self, power):
        self._resource.write(f"SOUR:POW:PROT {power}")
        return None

    def get_opp(self):
        return float(self._resource.query("SOUR:POW:PROT?").strip("W\n"))
